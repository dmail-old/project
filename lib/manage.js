...
var NodeProcess = Function.create({
	...
	setConfig: function(config){
		if( this.hasOwnProperty('config') ){
			if( this.config.events ){
				Object.keys(config.events).forEach(function(eventName){
					this.removeListener(eventName, config.events[eventName]);
				}, this);
			}
		}
		...
		if( config.args ){
			this.args = Object.assign({}, config.args, this.passedArgs);
		}
	...
	start: function(){
		if( this.state == 'started' || this.state == 'restarting' ){
			throw new Error('cannot start, wrong process state' + this.state);
		}
